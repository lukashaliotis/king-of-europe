...
import { json, cleanName, loadDataset } from "../_lib/util.js";
...
export async function onRequestPost({ request, env }) {
  ...
  const { board, name, uid, seed, startFive, arena, choices } = body || {};
  ...
  await env.DB.prepare(
    `INSERT INTO dynasty_scores (board, uid, name, streak, seed, created_at)
       VALUES (?1, ?2, ?3, ?4, ?5, ?6)
       ON CONFLICT(board, uid) DO UPDATE SET
         name = excluded.name, streak = excluded.streak, seed = excluded.seed, created_at = excluded.created_at
       WHERE excluded.streak > dynasty_scores.streak`
  ).bind(board, uid, dispName, res.streak, (Number(seed) || 0) >>> 0, Date.now()).run();

  const boardData = await dynastyBoardFor(env, board, uid);
  return json({ ok: true, streak: res.streak, ...boardData });
}
