...
export async function onRequestPost({ request, env }) {
  ...
  const { dayKey, name, uid, starters, sixth, coach } = body || {};
  ...
  await env.DB.prepare(
    `INSERT INTO scores (day_key, uid, name, wins, losses, stage, label, created_at)
       VALUES (?1, ?2, ?3, ?4, ?5, ?6, ?7, ?8)
       ON CONFLICT(day_key, uid) DO NOTHING`
  ).bind(dayKey, uid, dispName, result.wins, result.losses, result.stage, result.label, Date.now()).run();
